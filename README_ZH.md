...
- **版本无关的运行时跳板**
  跳板由 LDE 重定位引擎在运行时生成——逐指令解码、RIP-relative 重定位（±2GB 钳制）、相对分支拒绝、生成后按 CPU 视角回扫自检。没有绑定某个 Windows 构建的硬编码 prologue：钩子可跨 Windows 版本安装；不可重定位的 prologue 在安装时即被拒绝。
...
核心路径已在嵌套 AMD SVM（2 核 VMware 客户机，Windows 10 x64，v0.3c）完成实测：

| 能力 | 实测证据 |
|---|---|
| 全核 SVM 接管 | 2 核逐核 vmrun → 探针自证（'W'/'Q' 环）→ 干净卸载，多轮稳定 |
| 双 NPT 视图构建 | 1028 页双恒等树，覆盖 512GB，横幅打印双 NCR3 |
| 稳态零 exit 拦截 | 26081 次 NtClose 命中；NPF exit 计数器全程恒为 2（每核一例） |
| NPF 视图切换引擎 | 两核均在钩子页首次取指时切入 Secondary（错误码 0x100000015 = P+ID 位，与布防形态精确吻合）；此后零 NPF |
| detour 完整控制权 | 分发/CallOriginal/demo 计数经环面包屑（'V'/'H'/'h'/'O'）全链确认 |
| 版本无关跳板 | LDE 回扫自检通过；NtClose 重定位序言 22 字节 |
| 干净移除 | CodePage 字节还原 + 双视图 PTE 恒等还原 + 全核 TLB 同步；Remove 后新命中即刻停止 |
| 原子卸载 | 两核 STOP 桥 SVME 回读 = 0；泄漏核为零；NPT 页全额释放（1030 = 1028 + 2 拆分页，账目精确） |
...
里程碑 M0（骨架 + 观测体系）、M1（SVM 世界开关）、M2（NPT 恒等映射）与 M3（双 NPT 钩子引擎）已在**嵌套虚拟化**上分阶段实测毕业。待办里程碑：M4 单步原语、M5 隐身套件（CPUID / TSC 时间轴 / SVM 指令面 / NPT 自我隐蔽）、M6 时钟域、M7 root 加固与发布。本框架为研究级质量：hypervisor 层的缺陷仍可能导致蓝屏——**请务必在可弃置的测试机上运行**。
